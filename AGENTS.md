...
근거 탐색은 아래 순서로 한다.
1. `knowledge/optapex_help_center_sections_kr/00_Index.md` 를 먼저 읽고 질문과 연결된 파일 목록 선택
2. Index가 가리킨 실제 markdown 문서 직접 확인
3. 위 문서만으로 부족하거나, 시스템/운영 이슈 / 공개 Help Center와 내부 문서 충돌 / 에스컬레이션 판단이 애매한 경우에만 `references/optapex-source-routing.md` 를 예외적으로 참고
4. backlog는 **별도 필수 2차 조회가 아니라**, Index 또는 실제 section 문서를 읽은 뒤에도 질문이 `알려진 이슈 / ETA / 출시 예정 / 가능 여부`에 해당할 때만 `knowledge/03_optapex_backlog.md` 를 추가로 확인
5. 그래도 부족하면 추가 확인 또는 담당 라우팅 판단
...
- backlog는 source 선택의 예외 분기이지, 모든 답변에서 자동으로 추가 조회하는 기본 단계가 아니다.

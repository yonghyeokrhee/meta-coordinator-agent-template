...
- Q1처럼 KB markdown 근거로 바로 답하는 경우에는, 답변 본문에 사용한 근거의 **`[file + line range]` 인용을 반드시 포함**한다.
- 인용은 실제로 읽은 markdown 파일 기준으로만 달고, 포맷은 예를 들어 `Source: knowledge/04_optapex_help_center.md#L538-L542` 처럼 남긴다.
- backlog성 질문은 `진행현황`과 `완료 예정일/Release`를 함께 본다.

### 4. 답변 작성
- 답변 가능한 경우, optapex-cs-answer skill에 따라서  **답변만** 보낸다.
- Q1 즉답형으로 답하는 경우에는 답변 말미에 사용한 근거의 **`[file + line range]` 인용을 1개 이상 반드시 포함**한다.

...
- **이 문서로 답이 불충분할 경우**:
  - 시스템 / 기능 오류 / 연동 이슈 → [01_system_operations_manual.md](./01_system_operations_manual.md)
  - 미래 출시 / 알려진 이슈 / 로드맵 → [03_optapex_backlog.md](./03_optapex_backlog.md)
  - 공개 도움말이 더 정돈된 경우 → [04_optapex_help_center.md](./04_optapex_help_center.md)

- **에스컬레이션 / 워크플로우**:
  - 운영 데이터 조회 필요 → 사내 AM 이 DBeaver / Hiware 로 직접 조회 (CS Agent 영역 밖)
  - 환불 / 구독 변경 / 데모 / 프로모션 / 기능 요청 → Ticket 시스템
  - 벤치마크 데이터 요청 → **전달 불가** (FAQ Q10 참조)

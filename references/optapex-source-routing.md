...
- backlog를 포함한 다른 canonical source는 **자동 후속 조회 대상이 아니라**, section 문서만으로 부족할 때만 추가로 연다.
...
3. Index가 가리킨 section 문서를 읽고도, 알려진 이슈 / ETA / 출시 예정 / 가능 여부 확인이 남아 있어 `03_optapex_backlog.md` 를 추가로 열어야 하는 경우
...
- backlog 질문이라고 해서 이 문서를 먼저 열지 않는다. 기본값은 `00_Index.md` → 선택된 section 문서 → 필요 시 backlog 직접 확인이다.

...
- 이미 등록된 CS에 **직접적으로 연결되는 후속 질문/후속 답변**은 같은 `CS No`의 `Notes` 또는 답변 기록에 append 한다.
- 직접 연결 여부는 다음 기준으로 본다.
  - 같은 symptom / 같은 기능·페이지 / 같은 source / 같은 다음 액션을 이어가는가
  - 기존 답변의 보강, 정정, 재확인 성격인가
- 아래 경우는 append 하지 않는다.
  - agent rule, workflow, 운영 기준을 묻는 메타 질문
  - 기존 CS와 무관한 새 질문
  - 질문 축이나 bucket이 달라 별도 triage가 필요한 경우

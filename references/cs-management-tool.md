...
- 후속 질문/후속 답변처럼 보여도 기본값은 기존 `CS No`에 append 하지 않는다.
- 기존 케이스와 관련이 있어 보여도 우선 새 CS로 분리하여 관리한다.
- agent rule, workflow, 운영 기준을 묻는 메타 질문은 물론이고, CS 본문 후속처럼 보이는 질문도 자동 append 대상이 아니다.
- 즉 현재 운영 기본값은 `append`가 아니라 `새 CS 분리`다.

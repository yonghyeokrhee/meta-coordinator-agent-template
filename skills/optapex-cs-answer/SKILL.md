...
9. `Optapex Manual`의 Knowledge Map에서 질문 축을 찾는다.
10. 연결된 실제 markdown 문서를 직접 읽는다.
11. 필요하면 backlog를 확인한다.
...
- KB markdown 근거로 바로 답하는 경우, 답변에 사용한 근거의 **`[file + line range]` 인용을 반드시 포함**한다.
- 인용은 실제로 읽은 파일 기준으로만 달고, 포맷은 `Source: knowledge/04_optapex_help_center.md#L538-L542` 형태를 기본으로 한다.
